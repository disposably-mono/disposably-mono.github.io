...
var CONTACT_ICONS = {
  ...
  mail: '<svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M20 4H4c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zm0 4l-8 5-8-5V6l8 5 8-5v2z"/></svg>',
};
...
function createContactMat() {
  ...
  const labels = { facebook: 'Facebook', instagram: 'Instagram', linkedin: 'LinkedIn', github: 'GitHub', mail: 'Email' };

  const links = CONTACT_DATA.links
    .filter((link) => link.label !== 'mail')
    .map((link) => {
      return `<a class="contact-icon-link contact-icon--${link.label}" href="${link.href}" target="_blank" rel="noopener" aria-label="${link.label}" style="--icon-color: ${link.color}">${CONTACT_ICONS[link.label]}<span class="contact-icon-label">${labels[link.label]}</span></a>`;
    }).join('');
    ...
    <div class="contact-tablet">
      <div class="contact-screen">
        ...
        <div class="contact-panel-identity">
          ...
          <a class="contact-resume-link" href="public/resume/resume.pdf" target="_blank" rel="noopener">view resume</a>
        </div>
        ...
        <div class="contact-panel-note">
          ...
          <a class="contact-cta" href="mailto:mikel.taopa@gmail.com">${CONTACT_ICONS.mail}<span>Write to Mikel &rarr;</span></a>
